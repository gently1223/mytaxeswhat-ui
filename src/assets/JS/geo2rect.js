(function (global, factory) {
  ...
  function compute(data) {
    ...
    data.features.forEach(function (d, di) {
      ...
      var _loop = function _loop(i) {
        ...
        geom.forEach(function (dd, ddi) {
          var t_dist = Math.abs(
            Math.sqrt(Math.pow(pc[0] - dd[0], 2) + Math.pow(pc[1] - dd[1], 2))
          );
          if (
            t_dist < dist &&
            (ddi < corners[0] ||
              ddi > corners[corners.length - 1] ||
              corners.length === 0)
          ) {
            dist = t_dist;
            corner = ddi;
    ...
    function largestPoly(geom) {
      ...
      return [
        geom.type === "MultiPolygon"
          ? geom.coordinates[poly][0]
          : geom.coordinates[poly],
      ];
    }
          ...
          if (
            ...
            (function () {
              ...
              _this2._config.projection
                ...
                .scale(init_zoom)
                .translate([
                  _this2._config.width / 2,
                  _this2._config.height / 2,
                ]);
              ...
              var bounds = _this2._path.bounds(_this2._data),
                ...
                scale = Math.max(
                  ...
                  0.9 /
                    Math.max(
                      dx / (_this2._config.width - 2 * _this2._config.padding),
                      dy / (_this2._config.height - 2 * _this2._config.padding)
                    )
                );

              _this2._config.projection.scale(init_zoom);
          ...
          if (this._init) {
            (function () {
              ...
              _this3._svg
                .selectAll("path")
                .transition()
                .duration(_this3._config.duration)
                .style("fill", "white")
      ...
      },
      {
        ...
        set: function set(c) {
          if (c) {
            ...
            this._col_size =
              (this._config.width - this._config.padding * 2) / this._rows;
            this._row_size =
              (this._config.height - this._config.padding * 2) / this._cols;
            ...
            for (var _g in this._config.grid) {
              this._config.grid[_g]["ox"] =
                this._config.width / 2 -
                (this._cols / 2) * this._col_size +
                this._col_size / 2;
              this._config.grid[_g]["oy"] =
                this._config.height / 2 -
                (this._rows / 2) * this._row_size +
                this._config.grid[_g].y * this._row_size +
                this._row_size / 2;
            }
